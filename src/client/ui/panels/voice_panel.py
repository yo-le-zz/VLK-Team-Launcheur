...
from PySide6.QtWidgets import (
    ...
    QFrame, QScrollArea, QFileDialog, QSizePolicy, QMessageBox
)
    ...
    def __init__(self, user_id: str, username: str, is_self: bool = False, parent=None):
        ...
        if not is_self:
            ...
            self.mute_btn.toggled.connect(
                lambda checked: self.mute_toggled.emit(self.user_id, checked)
            )
...
class VoicePanel(QWidget):
    ...
    def _build_ui(self):
        ...
        title = QLabel("🎙  VOCAL")
        title.setStyleSheet(
            f"font-size: 13px; font-weight: 800; letter-spacing: 2px; color: {TEXT_PRIMARY};"
        )
        ...
        clan_lbl.setStyleSheet(f"""
            ...
            color: {ACCENT_CYAN}; background: {BG_SURFACE};
            padding: 6px 0; border-bottom: 1px solid {BG_BORDER};
        """)
        ...
        self.mute_btn = self._ctrl_btn("🎙", "Micro (activé)", STATUS_GREEN)
        self.mute_btn.clicked.connect(self._toggle_mute)

        self.deaf_btn = self._ctrl_btn("🔊", "Son (activé)", STATUS_GREEN)
        self.deaf_btn.clicked.connect(self._toggle_deaf)

        self.avatar_btn = self._ctrl_btn("🖼", "Changer photo de profil", TEXT_MUTED)
    ...
    def _join_voice(self):
        ...
        if not ok:
            # Show a clear error dialog instead of just a dot
            QMessageBox.warning(
                self,
                "Erreur Vocal",
                f"Impossible de démarrer le chat vocal :\n\n{err}\n\n"
                "Vérifiez que votre microphone est branché et autorisé\n"
                "dans les paramètres de confidentialité du système.",
            )
            self._engine = None
            return
        ...
        self.join_btn.setStyleSheet(f"""
            QPushButton {{
                background: {STATUS_RED}; color: white; border: none;
        ...
        self.conn_dot.setStyleSheet(
            f"color: {STATUS_GREEN}; font-size: 10px; font-weight: 700;"
        )
        self.api.send_ws({
            "type": "voice_join",
    ...
    # ── Avatar picker — works on Windows and macOS ────────────────────────────

    def _pick_avatar(self):
        """
        Open a native file dialog to select a profile picture.
        QFileDialog works natively on both Windows (Win32) and macOS (NSOpenPanel).
        """
        path, _ = QFileDialog.getOpenFileName(
            self,
            "Choisir une photo de profil",
            os.path.expanduser("~"),   # Start in home dir — avoids permission issues
        ...
        if px.isNull():
            QMessageBox.warning(
                self, "Erreur", "Impossible de charger cette image.\nFormat non supporté."
            )
            return
        ...
        # Persist avatar_url as a local file:// URI
        ...
        try:
            ...
            pass  # Avatar still shows locally even if save fails
